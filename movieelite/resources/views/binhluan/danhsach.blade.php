...
<p><a href="{{ route('binhluan.them') }}" class="btn btn-info"><i class="fa-light fa-plus"></i> Thêm mới</a></p>
...
<span class="d-block fw-bold text-primary"><a href="{{ route('binhluan.sua', ['id' => $value->id]) }}">{{ $value->BaiViet->tieude }}</a></span>
...
<a href="{{ route('binhluan.kiemduyet', ['id' => $value->id]) }}">
...
<a href="{{ route('binhluan.kichhoat', ['id' => $value->id]) }}">
@if($value->kichhoat == 1)
...
<a href="{{ route('binhluan.sua', ['id' => $value->id]) }}">
<i class="fa-light fa-lg fa-edit"></i>
</a>
</td>
<td class="text-center"><a href="{{ route('binhluan.xoa', ['id' => $value->id]) }}" onclick="return confirm('Bạn có muốn xóa bình luận của bài viết {{ $value->BaiViet->tieude }} không?')">
