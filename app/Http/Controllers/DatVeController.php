...
public function postVNPay(Request $request)
{
    ...
    public function getDatVe(Request $request, $phim_id)
    {
        if (Auth::check()) {
            ...
            $phim = Phim::select('id', 'tenphim', 'hinhanh', 'gioihantuoi')->find($phim_id);

            $suatchieu = SuatChieu::with('phongchieu.rapchieu')->where('phim_id', $phim_id)->get();

            // Calculate the user's age
            $ngaysinh = date('Y', strtotime(Auth::user()->namsinh));
            $namhientai = date('Y');
            $tuoi = $namhientai - $ngaysinh;

            if ($tuoi >= $phim->gioihantuoi) {
                return view('booking.datve', compact('suatchieu', 'phim'));
            } else {
                return view('booking.kiemtratuoi');
            }
        } else {
